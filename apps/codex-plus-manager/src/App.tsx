...
function SettingsScreen({
  ...
  const stepwiseAutoProtocolHelp = t(
    "自动兼容会按 Chat Completions、Responses、Anthropic Messages 的顺序尝试；端点不存在、响应为空或格式不匹配时切换下一种接口。鉴权、限流和服务错误不会自动切换。",
  );
  return (
    <>
      <Panel>
        ...
        <CardContent>
          ...
          <div className="settings-block stepwise-settings-block" id={SETTINGS_STEPWISE_SECTION_ID}>
            ...
            <div className="form-row">
              <Field label={t("接口协议")}>
                <AppSelect
                  ...
                  title={form.codexAppStepwiseProtocol === "auto" ? stepwiseAutoProtocolHelp : ""}
                  options={[
                    { value: "chat_completions", label: "Chat Completions" },
                    { value: "responses", label: "Responses" },
                    { value: "anthropic_messages", label: "Anthropic Messages" },
                    { value: "auto", label: t("自动兼容"), title: stepwiseAutoProtocolHelp },
                  ]}
                />
              </Field>
              <Field label={t("模式")}>
            ...
            <div className="toolbar stepwise-settings-actions">
              <Button variant="secondary" onClick={() => void actions.testStepwiseSettings(form)}>{t("测试")}</Button>
              <Button onClick={() => void actions.saveSettings()}>{t("保存")}</Button>
            </div>
          </div>
...
function normalizeStepwiseProtocol(value: StepwiseProtocol | undefined): StepwiseProtocol {
  if (value === "responses" || value === "anthropic_messages" || value === "auto") return value;
